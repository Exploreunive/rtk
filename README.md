...
Legacy `[tee]` config sections are still honored: they map to `mode = "tee"` (file-based recovery, same behavior as before), or `mode = "disabled"` if you had `enabled = false`.
